...
public class GetContractListForTenantTest {
    ...
    private static final String CSV_FILE = "get_contract_list_for_tenant_test_results.csv";
    ...
    public void testGetContractListForTenant_Success() {
        ...
        String message = "";
        String testCaseId = "CON_TENANT_GET01";

        try {
            ResponseEntity<List<Contract>> response = contractController.getContractListForTenant(null, TEST_USERNAME);
            ...
            assertTrue(contract.getTenant_list().stream().anyMatch(t -> TEST_USERNAME.equals(t.getUsername())), "Tenant username should match");
        } catch (Exception e) {
            result = "FAILED";
            message = e.getMessage();
        }
        testResults.add(new String[]{testCaseId, "Contract/ContractController", "getContractListForTenant",
                "Kiểm tra lấy danh sách contract thành công với username hợp lệ",
                "username=\"" + TEST_USERNAME + "\"",
                "HTTP Status: 200 OK. Response: List<Contract> không rỗng, chứa contract với tenant username=" + TEST_USERNAME,
                message, result, ""});
    }

    @Test
    public void testGetContractListForTenant_NoDataFound() {
        String result = "PASSED";
        String message = "";
        String testCaseId = "CON_TENANT_GET02";

        try {
            ResponseEntity<List<Contract>> response = contractController.getContractListForTenant(null, "nonexistent");
            assertEquals(HttpStatus.OK, response.getStatusCode(), "HTTP status should be OK");
            List<Contract> contracts = response.getBody();
            assertNotNull(contracts, "Contract list should not be null");
        ...
        testResults.add(new String[]{testCaseId, "Contract/ContractController", "getContractListForTenant",
                "Kiểm tra lấy danh sách với username không tồn tại",
    ...
    public void testGetContractListForTenant_EmptyUsername() {
        String result = "PASSED";
        String message = "";
        String testCaseId = "CON_TENANT_GET03";

        try {
            ResponseEntity<List<Contract>> response = contractController.getContractListForTenant(null, "");
            assertEquals(HttpStatus.OK, response.getStatusCode(), "HTTP status should be OK");
            List<Contract> contracts = response.getBody();
            assertNotNull(contracts, "Contract list should not be null");
            assertTrue(contracts.isEmpty(), "Contract list should be empty");
        ...
        testResults.add(new String[]{testCaseId, "Contract/ContractController", "getContractListForTenant",
                "Kiểm tra lấy danh sách với username rỗng",
                "username=\"\"",
